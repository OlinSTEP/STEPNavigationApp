...
class CloudAnchorAligner {
    ...
    func adjust(currentAlignment: simd_float4x4?)->simd_float4x4? {
        var proposedAlignments: [String: simd_float4x4] = [:]
        for (cloudID, resolutions) in resolvedCloudAnchors {
            ...
            proposedAlignments[cloudID] = anchorTransform.alignY() * mapTransform.alignY().inverse
        }
        ...
        var votes: [(Int, simd_float4x4)] = []
        for (_, proposedAlignment) in proposedAlignments {
            var vote = 0
            var inlierPoses: [simd_float4x4] = []
            inlierPoses.append(proposedAlignment)
            let projectedPosition = proposedAlignment * currentDevicePose
            for (_, proposedAlignment2) in proposedAlignments {
                let projectedPosition2 = proposedAlignment2 * currentDevicePose
                if simd_distance(projectedPosition.columns.3,
                                 projectedPosition2.columns.3) < 2.0 {
                    vote += 1
                    inlierPoses.append(proposedAlignment2)
                }
            }
            
            votes.append((vote, average(poses: inlierPoses)!))
        }
        
        // choose the one with the most votes.  If there are no votes, just stick with what we had originally
        return votes.max(by: { $0.0 < $1.0 })?.1 ?? currentAlignment
    }
