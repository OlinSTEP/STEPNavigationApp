    ...
    private func startRecordingCloudAnchors(withFrequency hz: Double) {
        cloudAnchorTimer = Timer.scheduledTimer(withTimeInterval: 1/hz, repeats: true) { timer in
            ...
            let _ = PositioningModel.shared.createCloudAnchorFromBufferedPose(
                withMetadata: CloudAnchorMetadata(name: "", type: .path, associatedOutdoorFeature: "", geospatialTransform: GeospatialData(arCoreGeospatial: geospatialTransform), creatorUID: AuthHandler.shared.currentUID ?? "", isReadable: true, organization: "", notes: "")
            ) { wasSuccessful in
